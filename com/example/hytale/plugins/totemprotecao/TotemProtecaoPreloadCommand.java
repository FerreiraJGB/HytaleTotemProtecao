...
public final class TotemProtecaoPreloadCommand extends AbstractAsyncCommand {
   ...
   public TotemProtecaoPreloadCommand(TotemProtecaoPlugin plugin) {
      super("preload", "Reloads TotemProtecao recharge items");
      this.plugin = plugin;
   }
   ...
   protected CompletableFuture<Void> executeAsync(CommandContext ctx) {
      if (!this.isAllowed(ctx)) {
         ...
         ctx.sendMessage(Message.raw("Plugin unavailable"));
         return CompletableFuture.completedFuture((Void) null);
      } else {
         boolean ok = this.plugin.reloadRechargeItemsFromDisk(true);
         if (!ok) {
            ctx.sendMessage(Message.raw("Failed to reload " + this.plugin.getRechargeConfigRelativePath()));
            return CompletableFuture.completedFuture((Void) null);
         } else {
            List<TotemProtecaoPlugin.RechargeItemConfig> items = this.plugin.getRechargeItemConfigs();
            ctx.sendMessage(Message.raw("Recharge items reloaded: " + items.size() + "/2"));
